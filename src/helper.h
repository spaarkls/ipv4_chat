/**
 * @file helper.h
 * @brief Вспомогательные функции для работы с аргументами и проверкой IP
 *
 * Содержит:
 *  - функцию parse_args() для извлечения IP и порта из аргументов командной
 * строки
 *  - функцию ipIsValid() для проверки корректности IPv4 адреса
 *  - функцию portIsValid() для проверки корректности порта
 */

#ifndef __HELPER_H__
#define __HELPER_H__
#include "common/object.h"
#include "exceptions/exceptions.h"

/**
 * @brief Парсит аргументы командной строки в структуру NetworkData
 *
 * Извлекает IP адрес и порт из массива argv и проверяет корректность IP.
 * Если IP некорректный, выбрасывает исключение BadIp.
 *
 * @param argc Количество аргументов
 * @param argv Массив аргументов командной строки (argv[1] - IP, argv[2] - порт)
 * @return Структура NetworkData (IP и порт)
 * @throws BadIp Если переданный IP адрес, порт или количетсво аргументов
 * невалидно
 */
struct NetworkData parse_args(int argc, char* argv[]);

/**
 * @brief Проверяет корректность IPv4-адреса
 *
 * @param ip_address Строка с IP-адресом
 * @return true Если IP валиден
 * @return false Если IP невалиден
 */
bool ipIsValid(const std::string& ip_address);

/**
 * @brief Проверяет корректность порта
 *
 * @param port Строка с портом
 * @return true Если порт валиден
 * @return false Если порт невалиден
 */
bool portIsValid(const std::string& port);

#endif  // __HELPER_H__