...
inline void Network::set_network_data(
    const struct NetworkData& net_data) noexcept {
  data = net_data;
}
...
void Network::init_socket(void) {
  ...
  int reuse_opt = 1;
  if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &reuse_opt, sizeof(reuse_opt)) ==
      -1) {
    throw BadInitSocket("Error setsockopt with REUSEADDR!");
  }
  if (setsockopt(fd, SOL_SOCKET, SO_REUSEPORT, &reuse_opt, sizeof(reuse_opt)) ==
      -1) {
...
void Chat::send_message(const std::string& msg) {
  ...
  std::string message =
      ("[" + net.get_network_data().ip + "] " + data.nickname + ": " + msg);
  if (message.size() > MAX_MESSAGE_LEN) {
    ...
    sendto(net.get_fd(), message.c_str(), message.size(), 0,
           (const sockaddr*)&send_addr, sizeof(send_addr));
  }
}
...
void Chat::receive_message(void) {
  ...
  int count = recvfrom(net.get_fd(), buffer, sizeof(buffer), 0,
                       (sockaddr*)&recv_addr, &send_len);
  std::lock_guard<std::mutex> lock(mtx);
  if (count == -1) {
    const char* str = strerror(errno);
    std::cerr << "Error failed receive message! " << str << " " << net.get_fd()
              << std::endl;
